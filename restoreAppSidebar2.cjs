...
// Delete the mangled lines (we know they are around index 74, let's find the exact ones)
for(let i=70; i<80; i++) {
  if (lines[i].includes('          </div>') && lines[i+1].includes('CommentButton targetId="side-unassigned-header"')) {
    // lines[i-1] should be `    </div>`
    lines.splice(i-1, 4, replacement);
    break;
  }
}
