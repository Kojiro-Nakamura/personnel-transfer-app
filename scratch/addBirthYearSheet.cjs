...
// We need a regex since there are multiple occurrences of addListSheet.
// Actually, let's just do standard replace, since exportListToExcel is separate.
// In exportListToExcel:
code = code.replace(
...
// In exportPlanToExcel:
code = code.replace(
  /addListSheet\(workbook, '職員一覧', fileName, targetYear, employees, departments\);/,
  "addListSheet(workbook, '職員一覧', fileName, targetYear, employees, departments);\n    addBirthYearSheet(workbook, '生年別一覧', targetYear, employees, departments);"
);

// In exportUnifiedExcel:
code = code.replace(
