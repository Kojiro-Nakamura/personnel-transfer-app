...
const t2 = `  const handleSubmit = (e) => {
    e.preventDefault();
    onSave(fd);
  };`;
const r2 = `  const handleSubmit = (e) => {
    e.preventDefault();
    onSave(fd);
  };
...
const r3 = `              <FormInput label={\`生年月日\${getEraStr(fd.birthDate)}\`} type="date" value={fd.birthDate} onChange={v => setFd({...fd, birthDate: v})} className="w-[125px] shrink-0" />
              <FormInput label="学歴" value={fd.education} onChange={v => setFd({...fd, education: v})} className="flex-1 min-w-0" />
              <FormInput label={\`採用年月\${getEraStr(fd.hireDate)}\`} type="date" value={fd.hireDate} onChange={v => setFd({...fd, hireDate: v})} className="w-[125px] shrink-0" />`;
