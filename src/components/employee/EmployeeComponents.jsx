...
export const EmployeeCell = ({ emp, isNext, isEmpty, onClick, isPost, moveProps, isConflict, hasPeer, isIncreased }) => {
    ...
    return (
      <div 
        ...
        className={cx(
          ...
          isNext && isIncreased && "border-t-[5px] border-b-[5px] border-blue-600 border-solid"
        )}
        title={isNext ? (isPickingMode ? "選択中の職員をここに配置します" : "ここへ配置する職員を選択します") : ""}
      >
        {!isNext ? '' : (
          <span className="flex items-center gap-1 px-2 py-0.5 rounded shadow-sm z-10 bg-amber-100 text-black font-bold border border-amber-200">
  ...
  const cellClasses = cx(
    ...
    bgClass,
    isNext && isIncreased && "border-t-[5px] border-b-[5px] border-blue-600"
  );
