      ...
      <header className="bg-[#3972ac] text-white shadow-md z-20 sticky top-0 border-b border-[#2d5f91]">
        <div className="flex justify-between items-center p-2 border-b border-[#4d86c2]">
          ...
          <div className="flex gap-2 items-center">
            ...
            <button onClick={() => openModal('saveFile', { type: 'excel_list', defaultName: currentFileName ? currentFileName.replace('.json', '') + '_職員一覧Excel' : baseFileName + '_職員一覧Excel' })} className="bg-emerald-500/30 hover:bg-emerald-500/50 border border-emerald-300 text-emerald-50 active:scale-95 transition-all px-3 py-1.5 rounded flex items-center justify-center text-xs font-bold" title="現在の職員一覧をExcelファイルとして保存する"><FileCode className="w-4 h-4 mr-1" />職員一覧Excel</button>
            <button onClick={() => openModal('saveFile', { type: 'json', defaultName: currentFileName ? currentFileName.replace('.json', '') : baseFileName })} className="bg-[#5f8cc5] hover:bg-[#5f8cc5]/80 text-white active:scale-95 transition-all px-3 py-1.5 rounded flex items-center justify-center text-xs font-bold border border-transparent" title="現在のデータをJSONファイルとして保存する"><DownloadCloud className="w-4 h-4 mr-1" />保存</button>
            <label className="bg-[#cccccc] hover:brightness-95 text-slate-800 active:scale-95 transition-all px-3 py-1.5 rounded cursor-pointer flex items-center justify-center text-xs font-bold shadow-sm border border-transparent" title="保存したJSONファイルを読み込む"><FolderOpen className="w-4 h-4 mr-1" />開く<input type="file" accept=".json" onChange={loadJSON} className="hidden" /></label>
          </div>
        </div>
