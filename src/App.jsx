      ...
      <header className="bg-slate-800 text-white shadow z-20 sticky top-0">
        ...
        <div className="flex justify-between items-center bg-slate-900 px-2 pt-2">
          ...
          <div className="flex gap-2 pb-1.5 items-center">
            ...
            <button onClick={() => openModal('rollOver')} className="bg-orange-600 hover:bg-orange-500 active:scale-95 transition-all px-3 py-1 rounded text-xs font-bold ml-2" title="来年度の配置を今年度に確定し、新しい年度へ移行する">次年度移行</button>
          </div>
        </div>
      </header>
        ...
        return (
          <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-[#0F828C] text-white px-6 py-4 rounded-xl shadow-2xl z-[150] flex flex-col gap-3 border-2 border-white min-w-[480px]">
            ...
            <div className="bg-white text-slate-900 rounded px-3 py-2 flex items-center gap-3 shadow-inner">
              ...
              <div className={cx("w-16 text-[12px] text-right font-medium truncate shrink-0", ys >= 3 ? "text-rose-700 bg-rose-100 px-1 rounded" : "text-slate-800")} title={yd}>{yd}</div>
              <div className="w-20 truncate text-[11px] text-slate-700 text-left shrink-0 ml-1" title={noteText}>{noteText}</div>
            </div>
          </div>
        );
      })()}
