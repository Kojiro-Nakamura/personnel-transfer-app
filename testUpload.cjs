...
const buffer = fs.readFileSync('C:/Users/gyrom/.gemini/antigravity/brain/6d803aee-cb97-45dd-8cf9-5ce09ab6e882/.user_uploaded/media__1785294103771.csv');
...
console.log("Years mapped:", csvYearsMap.size);
...
console.log("EmpNum:", getVal('職員番号'));
console.log("Name:", getVal('氏名'));
console.log("pChief:", getVal('【昇進年度】係長級(主査)'));
...
for (let [k, year] of csvYearsMap.entries()) {
  if (k < cols.length) {
    ...
    if (deptName) {
      history.push({ year, department: deptName });
    }
  }
}
